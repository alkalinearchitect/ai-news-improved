#!/usr/bin/env python3
"""
AI News Scraper using browser automation (stealth mode)
Bypasses Cloudflare/bot protection by using a real browser.
"""
import requests
import re
import json
...
# Use the cached data we already have from web_extract + web_search
# Plus scrape additional articles via browser_exec
...
# Articles we already have from the scrape
ARTICLE_URLS = [
    "https://www.artificialintelligence-news.com/news/nvidia-circular-financing-ai-labs/",
    "https://www.artificialintelligence-news.com/news/gatik-200m-ai-autonomous-freight/",
    "https://www.artificialintelligence-news.com/news/nvidia-jetson-orin-nano-2-physical-ai-to-drones-and-robots/",
    "https://www.artificialintelligence-news.com/news/mit-ai-forecasts-extreme-weather-without-historical-data/",
    "https://www.artificialintelligence-news.com/news/xpeng-iron-humanoid-robot-draws-record-physical-ai-funding/",
    ...
    "https://www.artificialintelligence-news.com/news/ai-data-centre-regulation-pennsylvania-template/",
    "https://www.artificialintelligence-news.com/news/openai-president-urges-enterprises-hasten-ai-security-defences/",
]


def create_article_data(url, title, author, date, description, body, image, tags, categories):
    """Create structured article data."""
    return {
        "url": url,
        "title": title,
        "author": author or "Staff Writer",
        "date": date or "August 2026",
...
def build_articles_from_cache():
    """Build article data from the cached web_extract results we already have."""
    articles = []
    
    # Article 1: Nvidia circular financing
    articles.append(create_article_data(
...
Cognex, Doosan Bobcat, and Matic sit among the first companies NVIDIA names as adopting and exploring Jetson Orin Nano 2. NVIDIA says more than three million developers already build on its robotics stack. The company expects partners — including the aforementioned partners — to bring edge AI into home robots, vision AI systems, delivery and inspection drones, carrier boards, hardware systems, and reference designs.
...
Navneet Dalal, Cofounder and Chief Executive of Matic Robots, explained: "Home robots need to understand people, map spaces precisely, understand the layout of objects and spaces, and clean autonomously in dynamic and constantly changing environments.
...
## Carrier boards and reference designs

NVIDIA also named a large roster of Jetson hardware partners building around the new board.

AAEON, ADLINK, Advantech, and Aetina sit among the manufacturers building carrier boards and hardware systems for it. Antmicro, Aptiv, Auvidea and AVerMedia join a further group working on customised AI software and reference designs, alongside Chuanglebo, Connect Tech, ForeCR, and JWIPC.
...
However, Xpeng's shares are down 7.22 percent at the time of writing, and the 12-month decline stands at 51.24 percent. XPENG's core vehicle business faces intensifying competition inside China from domestic manufacturers, and from Tesla in both overseas and domestic markets. Investors are pricing the robotics division at $6.3 billion while the parent company's shares keep falling.
...
IDG Capital said the physical AI industry is moving "from technical breakthroughs to scalable manufacturing and commercial deployment," a shift it argues plays to XPENG's strengths. The firm pointed to XPENG's combination of edge AI processors, physical AI foundation models, and complete robotic systems, and to the technology links between the robotics unit and XPENG's electric vehicle and autonomous driving businesses.

Gaorong Ventures added that humanoid robots are "moving beyond demonstrations of mobility and dexterity toward reliable mass production and tangible value creation in real-world settings." The firm highlights XPENG's automotive-grade safety and quality standards as evidence the robot is built for commercial deployment and credited XPENG's decade-plus of supply chain and manufacturing experience in the EV sector.

He Xiaopeng commented: "I believe the strong capital backing from leading global and strategic investors provides the resources needed to accelerate the growth of our robotics business, while strengthening our ability to attract more world-class physical AI talent.

IRON brings together a highly human-like design, advanced AI intelligence, built to the highest standards of safety and quality. Our ambition is for IRON to become a trusted partner for people and a meaningful part of everyday work and life."
...
IRON is expected to enter mass production by the end of 2026, with customer deliveries planned for 2027 across China and overseas markets.""",
    ...
    # Article 6: Stripe OpenRouter acquisition
    articles.append(create_article_data(
...
OpenRouter says it processes more than 10 trillion tokens per day across a community of more than 10 million developers and companies."""
        ...
        categories=["AI Business Strategy", "AI Market Trends", "AI Mergers & Acquisitions", "Artificial Intelligence", "Featured News", "Features", "Infrastructure & Hardware", "Inside AI"]
    ))
    
    # Article 7: Amazon Prime Air drones
    articles.append(create_article_data(
...
Onboard cameras and sensors handle navigation, obstacle detection, and the delivery drop itself. Amazon has stated the cameras do not track individuals or record their movements, and the footage feeds only the drone's own navigation processing rather than a monitored video feed sent back to base. No person watches a live camera stream from any of the aircraft, according to the company.
...
First-time drone customers set a delivery point when they place that initial order, then can reuse it or pick a new spot for later purchases. Before releasing a package, the drone scans the delivery point for people, pets, or vehicles in the way.

Prime Air occupies one tier in a broader set of Amazon delivery options rather than replacing them. Amazon Now offers 30-minute delivery in dozens of US cities on thousands of grocery, household, and locally-relevant items, dispatched from smaller sites concentrated in more populated areas.

One-hour and three-hour delivery covers a selection of more than 90,000 items through Amazon's Same-Day Delivery network. Same-Day Delivery itself reaches millions of items across more than 10,000 cities and towns, including a growing number of smaller towns and rural areas.

Amazon views Prime Air as complementary to that logistics structure: drone delivery from Amazon's own sites, positioned primarily for suburban areas, sitting alongside denser urban same-day options and the wider Same-Day network's rural reach.
...
Amazon plans to launch Prime Air service soon in the Chicago, Illinois; Syracuse, New York; Cleveland, Ohio; Atlanta, Georgia; and Boise, Idaho metro areas, with further communities scheduled to follow later this year.""",
...
# Import the HTML generation functions from the main scraper
def generate_css():
    return """/* AI News - Clean Dark Theme - Built by OWL */
...
.featured-article::before { content: ""; position: absolute; inset: 0; background: linear-gradient(180deg, transparent 0%, rgba(10, 10, 11, 0.95) 100%); z-index: 1; }
...
""")
    ...
    if not featured:
        html_parts.append("</head><body><div class='container'><p>No articles available.</p></div></body></html>")
        return "".join(html_parts)
    
    slug = featured["url"].split("/")[-2]
    html_parts.append(f"""
...
<body>
  ...
  <main class="container">
    <section class="hero featured-article">
      {f'<img src="{featured["image"]}" alt="{featured["title"]}">' if featured.get("image") else ''}
...
""")
    ...
    # Category sections
    if by_category:
        html_parts.append('<section id="by-category">')
        html_parts.append('<h2 style="font-size:28px;font-weight:700;margin:32px 0 20px;">By Category</h2>')
        
        for cat, cat_articles in list(by_category.items())[:5]:
            html_parts.append(f'<h3 style="font-size:20px;font-weight:600;margin:24px 0 16px;color:#a8a8b0;">{cat}</h3>')
            html_parts.append('<div class="article-grid">')
...
def generate_article_page(article, all_articles):
    ...
    tags = ''.join(f'<a href="#">{t}</a>' for t in article.get("tags", [])[:10])
    
    # Split body into paragraphs/sections
    body_parts = article.get("body", "").split("\n\n")
    body_html = ""
    for i, para in enumerate(body_parts):
        if para.startswith("## "):
            ...
            clean_para = strip_html(para) if '<' in para else para
            body_html += f'<p{cls}>{clean_para}</p>\n'
    
    related_html = ""
    for rel in related:
        rel_slug = rel["url"].split("/")[-2]
        img = rel.get("image", "") or ""
        related_html += f"""
        <article class="article-card">
          {'<img src="' + img + '" alt="' + rel["title"] + '" loading="lazy">' if img else ''}
          <div class="card-content">
            <h3 style="font-size:17px;">{rel["title"]}</h3>
            <div class="meta" style="font-size:13px;"><span>{rel["date"]}</span></div>
...
<body>
  ...
  <article class="article-page">
    ...
    {'<img src="' + article.get("image", "") + '" alt="' + article["title"] + '" loading="lazy">' if article.get("image") else ""}
...
if __name__ == "__main__":
    ...
    articles = build_articles_from_cache()
    
    # Also try live scraping as a fallback enrichment
    print(f"Loaded {len(articles)} articles from latest AI news data")
    print()
